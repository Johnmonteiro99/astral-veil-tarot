(function () {
  ...
  const tarotGuides = [
    ...
    { key: "beginners", title: "Tarot for Beginners", description: "Learn the structure of the deck, basic card meanings, and the foundations of your first reading.", route: "/tarot/for-beginners/", available: true, ctaLabel: "Begin with Tarot Basics", image: "/assets/images/background%20_images/tarot-for-beginners.png", imageAlt: "A tarot deck, candle, crystals, and open guidebook arranged for a first reading", imageWidth: 1448, imageHeight: 1086, imageAspect: "landscape", imageFit: "cover", objectPosition: "68% center", layoutSize: "standard" },
    { key: "how-to-read", title: "How to Read Tarot Cards", description: "Learn how to form a question, choose a spread, draw cards, and interpret their message.", route: "/how-to-read-tarot-cards/", available: true, ctaLabel: "Learn to Read Tarot", image: "/assets/images/background%20_images/how-to-read-tarot.png", imageAlt: "Hands arranging tarot cards beside candles and crystals during a reading", imageWidth: 1448, imageHeight: 1086, imageAspect: "landscape", imageFit: "cover", objectPosition: "70% center", layoutSize: "standard" },
    ...
    { key: "tarot-vs-oracle", title: "Tarot vs. Oracle Cards", description: "Compare Tarot’s structured archetypes and traditional meanings with Oracle cards’ flexible, deck-specific symbolism and reading styles.", route: "/tarot/compare/tarot-vs-oracle-cards/", available: true, ctaLabel: "Compare Tarot and Oracle", image: "/assets/images/background%20_images/tarot-vs-oracle-cards.png", imageAlt: "Tarot and oracle cards arranged together beneath a crescent moon", imageWidth: 1672, imageHeight: 941, imageAspect: "wide", imageFit: "cover", objectPosition: "54% center", layoutSize: "comparison", imageSide: "left" },
    { key: "tarot-vs-lenormand", title: "Tarot vs. Lenormand", description: "Explore the differences between tarot’s archetypes and Lenormand’s direct symbolic language.", route: "/tarot/compare/tarot-vs-lenormand/", available: true, ctaLabel: "Compare Tarot and Lenormand", image: "/assets/images/background%20_images/tarot-vs-lenormand.png", imageAlt: "Tarot cards and Lenormand cards arranged side by side on a celestial cloth", imageWidth: 1672, imageHeight: 941, imageAspect: "wide", imageFit: "cover", objectPosition: "55% center", layoutSize: "comparison", imageSide: "right" }
  ];
  ...
  function renderTarotGuides() {
    ...
    tarotGuidesGrid.innerHTML = tarotGuides.map((guide) => {
      ...
      const statusDecorations = guide.available
        ? '<span class="tarot-guide__spark" aria-hidden="true">✦</span><span class="tarot-guide__arrow" aria-hidden="true">→</span>'
        : "";
      const statusLabel = guide.layoutSize === "comparison"
        ? `<span class="tarot-guide__cta-label">${escapeHtml(status)}</span>`
        : escapeHtml(status);
      const content = `<div class="tarot-guide__content">${guide.eyebrow ? `<p class="tarot-guide__eyebrow">${escapeHtml(guide.eyebrow)}</p>` : ""}<h3 class="tarot-guide__title">${escapeHtml(guide.title)}</h3><p class="tarot-guide__description">${escapeHtml(guide.description)}</p><p class="tarot-guide__status tarot-guide__cta">${statusLabel}${statusDecorations}</p></div>`;
