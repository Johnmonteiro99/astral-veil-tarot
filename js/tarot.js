(function () {
  ...
  function renderTarotGuides() {
    ...
    tarotGuidesGrid.innerHTML = tarotGuides.map((guide) => {
      ...
      const content = `<div class="tarot-guide__content">${guide.eyebrow ? `<p class="tarot-guide__eyebrow">${escapeHtml(guide.eyebrow)}</p>` : ""}<h3>${escapeHtml(guide.title)}</h3><p class="tarot-guide__description">${escapeHtml(guide.description)}</p><p class="tarot-guide__status">${escapeHtml(status)}</p></div>`;
