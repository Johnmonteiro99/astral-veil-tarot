(function () {
  ...
  const faqSection = document.querySelector(".tarot-history-faq");

  if (faqSection) {
    const faqButtons = Array.from(faqSection.querySelectorAll("[data-history-faq-button]"));
    const faqPanels = Array.from(faqSection.querySelectorAll("[data-history-faq-panel]"));
    const faqFeatured = faqSection.querySelector("[data-history-faq-featured]");
    const faqStatus = faqSection.querySelector("[data-history-faq-status]");
    const faqTransitionMs = 520;
    let activeFaqIndex = 0;
    let faqTransitioning = false;

    function clearFaqMotion() {
      faqPanels.forEach((panel) => panel.classList.remove("is-entering", "is-leaving"));
    }

    function updateFaqState(index, { focusTab = false } = {}) {
      clearFaqMotion();
      activeFaqIndex = index;

      faqPanels.forEach((panel, panelIndex) => {
        const isActive = panelIndex === index;
        panel.classList.toggle("is-active", isActive);
        panel.setAttribute("aria-hidden", String(!isActive));
      });

      faqButtons.forEach((button, buttonIndex) => {
      ...
      if (faqStatus) {
        const question = faqButtons[index]?.querySelector(".tarot-history-faq__index-question")?.textContent?.trim() || "";
        faqStatus.textContent = `Question ${index + 1} of ${faqButtons.length} selected: ${question}`;
      }

      faqSection.classList.remove("is-transitioning");
      faqFeatured?.removeAttribute("aria-busy");
      faqTransitioning = false;
      if (focusTab) faqButtons[index]?.focus();
    }

    function waitForFaqFrame() {
      return new Promise((resolve) => {
        window.requestAnimationFrame(() => window.requestAnimationFrame(resolve));
      });
    }

    function waitForFaqTransition(panel) {
      return new Promise((resolve) => {
        let completed = false;
        const finish = () => {
          if (completed) return;
        ...
        const timeout = window.setTimeout(finish, faqTransitionMs + 120);
        panel.addEventListener("transitionend", onTransitionEnd);
      });
    }

    function scrollFaqAnswerIntoView(behavior = reducedMotionQuery.matches ? "auto" : "smooth") {
      if (!mobileFaqQuery.matches || !faqFeatured) return;
      faqFeatured.scrollIntoView({ behavior, block: "start" });
    }

    async function activateFaq(index, { focusTab = false, returnToAnswer = false } = {}) {
      const nextIndex = Math.max(0, Math.min(index, faqButtons.length - 1));
      if (faqTransitioning || nextIndex === activeFaqIndex) {
        if (focusTab && nextIndex === activeFaqIndex) faqButtons[nextIndex]?.focus();
        return;
      }

      if (reducedMotionQuery.matches) {
        updateFaqState(nextIndex, { focusTab });
        if (returnToAnswer) window.requestAnimationFrame(() => scrollFaqAnswerIntoView("auto"));
        return;
      }

      const outgoingPanel = faqPanels[activeFaqIndex];
      const incomingPanel = faqPanels[nextIndex];
      if (!outgoingPanel || !incomingPanel) return;

      faqTransitioning = true;
      faqSection.classList.add("is-transitioning");
      faqFeatured?.setAttribute("aria-busy", "true");
      incomingPanel.classList.add("is-entering");
      incomingPanel.setAttribute("aria-hidden", "false");
      if (returnToAnswer) scrollFaqAnswerIntoView();

      await waitForFaqFrame();
      ...
      await waitForFaqTransition(incomingPanel);
      updateFaqState(nextIndex, { focusTab });
    }
    ...
    faqButtons.forEach((button, index) => {
      button.addEventListener("click", (event) => {
        activateFaq(index, {
          returnToAnswer: mobileFaqQuery.matches && event.detail !== 0
        });
      });
      button.addEventListener("keydown", (event) => {
        let nextIndex = null;
        if (event.key === "ArrowRight" || event.key === "ArrowDown") nextIndex = (activeFaqIndex + 1) % faqButtons.length;
        if (event.key === "ArrowLeft" || event.key === "ArrowUp") nextIndex = (activeFaqIndex - 1 + faqButtons.length) % faqButtons.length;
        if (event.key === "Home") nextIndex = 0;
        if (event.key === "End") nextIndex = faqButtons.length - 1;
        if (nextIndex === null) return;
        event.preventDefault();
        activateFaq(nextIndex, { focusTab: true });
      });
    });

    faqSection.classList.add("is-faq-enhanced");
    updateFaqState(0);
  }
